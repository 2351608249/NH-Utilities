...
public class MTEMagicalEggMachine extends MTEBasicGenerator {
    ...
    private int getEggBonus() {
        ...
        if (block.getUnlocalizedName()
            .contains("creeperEgg")) return 128;
        if (block == Blocks.dragon_egg) return 512;
        if (block == NHUItemList.ChaosDragonEgg.getBlock()) return 2048;
        if (block.getUnlocalizedName()
            .contains("infinityegg")) return 8192;
        if (block == NHUItemList.AncientDragonEgg.getBlock()) return 32768;
        return 0;
    }
